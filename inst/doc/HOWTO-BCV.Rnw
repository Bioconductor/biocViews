...
<<lkd>>=
cat(readLines(system.file("dot/oct05.dot", package="biocViews")),sep="\n")
@
...
biocViews <- getBiocViews(reposUrl, oct05, "vocRoot")
...
Coming soon.  But basically, it will look like this:

<<html1, eval=FALSE>>=
viewsDir <- "testRepos"
for (bv in biocViews) {
    fn <- file.path(viewsDir, htmlFilename(bv))
    html <- htmlDoc(bv)
...
Given that we want to be able to generate views remotely, I'm not sure
how we want to deal with absolute vs relative links and such.  
